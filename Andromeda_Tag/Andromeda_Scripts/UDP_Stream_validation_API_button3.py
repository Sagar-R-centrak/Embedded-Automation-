...
for i in range(1):
    ...
    with open(file_path, "a") as file:
        ...
        if current_mA < 11.00:
            print("Normal current consumption", current_mA, "mA")
            Report("POR sleep current", 1)
            Output(f"Iteration{i}: POR sleep current Test PASS")
    ...
    logging.info("Executing LBI : To validate tag LBI value \n  ")
    time.sleep(3)
    expected_values = {
        'Tag ID': tag1_id,
        'LBI': 3000
    ...
    if result:
        print("Packet validation successful.")
        print(f"Iteration{i}: LBI Test PASS")
        Report("LBI", 1)
        Output(f"Iteration{i}: LBI Test PASS")
    else:
        print("Packet validation failed.")
        print(f"Iteration{i}: LBI Test FAIL")
        Report("LBI", 0)
        Output(f"Iteration{i}: LBI Test FAIL")
