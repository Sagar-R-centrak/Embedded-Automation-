...
class RateProcessor:
    ...
    @staticmethod
    def decode_status_byte1(status_byte):
    ...
    @staticmethod
    def decode_location_packet1(packet):
        ...
        status_fields = RateProcessor.decode_status_byte1(status_byte)
    ...
    @staticmethod
    def validate_packet(decoded_packet, expected_values):
        for key, expected_value in expected_values.items():
            if decoded_packet.get(key) != expected_value:
                return 0
        return 1

    def Intervals(self, Ctime, EXPECTED_VALUES, expected_validation_count, expected_report_rate):
        captured_packets = []
        timestamps = []
        intervals = []
        validation_count = 0
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
            ...
            while time.time() - start_time < Ctime:
                try:
        ...
        for i in range(0, len(captured_packets) - 1, 2):  # Process packets in pairs
            ...
            if len(combined_packet) > 13:
                ...
                data_length = self.decode_header1(header)
                ...
                for j in range(num_location_packets):
                    ...
                    decoded_packet = self.decode_location_packet1(location_packet)

                    if self.validate_packet(decoded_packet, EXPECTED_VALUES) == 1:
                        validation_count += 1
                        timestamps.append(decoded_packet['Timestamp'])

        for k in range(1, len(timestamps)):
            interval = abs((timestamps[k] - timestamps[k - 1]).total_seconds())
            intervals.append(interval)

        self.log_intervals(timestamps, intervals)

        report_rate = sum(intervals) / len(intervals) if intervals else 0
        print(f"Validation count: {validation_count} Report rate : {report_rate}")

        return abs(validation_count - expected_validation_count) <= 1 and abs(report_rate - expected_report_rate) <= 1
